# Itheum Core MultiversX - Itheum Liveliness Stake Contract

## Abstract

Liveliness stacking generates staking rewards for data creators who have bonded $ITHEUM tokens as part of the Itheum life bonding system.
...
- Note that this contract needs to host a "proxy" (liveliness_stake_proxy) so that the life-bonding-sc call interact with this. This proxy needs to be pre-generated and used here before we call `mxpy contract build` as we don't think mxpy build is auto generating a new proxy. The proxy used was generated by the engineer using `https://docs.multiversx.com/developers/meta/sc-meta-cli`, which seems to be what mxpy calls under the hood. SO MAKE SURE THAT ANY UPGRADES WE MAKE TO THIS SC, BEFORE DEPLOYING WE ALSO GENERATE A NEW PROXY FILE.
...
- Note the `REMOVE DEV DEPENDENCIES BEFORE FINAL BUILD` section above please.
...
1. Make sure your mxpy version is >= 6 (In v3.0.0, we used mxpy 9.6.0).
